...
async function processImportRow({
  ...
  return {
    ...
    commissionChanged,
    taxChanged,
    created: isNewCourier ? 1 : 0,
    updated: !isNewCourier && (commissionChanged || taxChanged) ? 1 : 0,
    skipped: !isNewCourier && !commissionChanged && !taxChanged ? 1 : 0,
  };
}
...
export async function confirmCommissionImport({
  ...
  for (let index = 0; index < rows.length; index += IMPORT_BATCH_SIZE) {
    ...
    const batchResults = await runTransaction(async (tx) => {
      const processed = [];
    ...
    for (const rowResult of batchResults) {
      ...
      if (rowResult.commissionChanged || rowResult.taxChanged) {
        affectedCourierIds.add(rowResult.courierId);
      }
      results.push({
        externalId: rowResult.externalId,
        name: rowResult.name,
        courierId: rowResult.courierId,
        isNewCourier: rowResult.isNewCourier,
        commission: rowResult.commission,
        tax: rowResult.tax,
        commissionChanged: rowResult.commissionChanged,
        taxChanged: rowResult.taxChanged,
      });
    }
  }
